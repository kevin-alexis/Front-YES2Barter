...
<template>
  <div class="flex flex-wrap items-center bg-white p-6 rounded-lg shadow-lg w-full max-w-4xl mx-auto mb-6 hover:shadow-xl transition-shadow duration-300">
    ...
    <div class="flex flex-wrap items-center w-full max-w-4xl mx-auto">
      ...
      <img
        :src="URL_API_SOURCE + ( isOfertante ? props.propuestaIntercambio.objetoOfertado.rutaImagen : props.propuestaIntercambio.objetoSolicitado.rutaImagen)"
    ...
    <div class="flex-1">
      <p class="font-semibold text-xl text-gray-800 mb-1">{{ props.propuestaIntercambio.objetoOfertado.nombre }}</p>
      <p class="font-light text-sm text-gray-800 mb-1">{{ props.propuestaIntercambio.objetoOfertado.descripcion }}</p>
    ...
    <div class="w-full flex gap-4 justify-center sm:justify-start mt-4" v-if="isInteractive && props.propuestaIntercambio.estado == EstatusPropuestaIntercambio.ENVIADA">
