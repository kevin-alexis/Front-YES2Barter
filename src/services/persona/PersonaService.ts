import {
  genericRequestAuthenticated,
  genericRequestFormDataAuthenticated,
} from '@/utils/genericRequest'
...
export class PersonaService extends BaseService<IPersona> {
  ...
  public getAllPersonasIntercambiadores = async () => {
    try {
      const response = await genericRequestAuthenticated(
        '/Persona/GetAllPersonasIntercambiadores',
        'GET',
      )
      return response
    } catch (error: unknown) {
  ...
  public getPersonaByIdUsuario = async (idUsuario: string) => {
    try {
      const response = await genericRequestAuthenticated(
        `/Persona/GetPersonaByIdUsuario/${idUsuario}`,
        'GET',
      )
      return response
    } catch (error: unknown) {
  ...
  public updateFotoPersona = async (id: number, rutaFotoPerfil: FormData) => {
    try {
      const response = await genericRequestFormDataAuthenticated(
        `/Persona/UpdateFotoPerfil/${id}`,
        'PUT',
        rutaFotoPerfil,
      )
      return response
      ...
      if (error instanceof Error) {
        this.logService.create({
          nivel: 'Error',
          mensaje: `Error en el método updateFotoPerfil: ${error.message}`,
          excepcion: error.toString(),
        })
        console.error(error)
      }
